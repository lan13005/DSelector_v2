...
string folder="/d/grid17/ln16/myDSelector/zDR_BA/baseCuts_DR/";
...
void fitAsymmetryPlots(string chosenEvtSelect=""){
    ...
    //   To check if  the selections is working in general we can look at fitAsymmetryPlots_results/diagnostics/ histograms
    ...
    eventSelects={
        ...
        {"_evtSel_ueL2", {make_pair(0,criteria{"unusedEnergy",fltmin,fltmax,fltmin,fltmin})} },
        {"_evtSel_chiT", {make_pair(1,criteria{"chiSq",fltmin,11,fltmin,fltmin})} },
        {"_evtSel_chiL", {make_pair(1,criteria{"chiSq",fltmin,40,fltmin,fltmin})} },
        {"_evtSel_gThetaBeamL", {   
        ...
        {"_evtSel_pMomT1", {make_pair(10,criteria{"proton_momentum",0.5,fltmax,fltmin,fltmin})} },
        {"_evtSel_pMomT2", {make_pair(10,criteria{"proton_momentum",0.6,fltmax,fltmin,fltmin})} },
...
//    string weightVar="weightASBS"; // Will not overwrite AccWeight if "AccWeight" else overwrite weightVar with AccWeight*weightBS
    ...
    for (auto const& ele: eventSelects){
        ...
        if ((chosenEvtSelect!="")*(evtTag!=chosenEvtSelect)) 
            continue;
        ...
        for (int i=0; i<(int)sbTags.size(); ++i){
            ...
            float percentage = extractAsymmetries(sbTags[i],sbRegions[i],evtTag,args,defSels);
            percentages[evtTag+sbTags[i]]=percentage;
        }
    }
    //if (showSummary){
    //    cout << endl;
...
void constructAndFit(
    map<string, float*> &variable_map, // holds all the variables of interest in the tree as values in a map. Used to fill array_variable_map 
    map<string, vector<float>> &array_variable_map, // contains all the data that passed the defaultSelections 
        ...
        for(int iData=0; iData<4; ++iData){
            ...
            for (auto phi: phis_pi0){
                ...
                if (phi->GetEntries()>minEntriesInPhiHists){
                    ...
                    if (fitPointer!=0){
                        if (verbose){ cout << "Shifted cos fit " << polnames[counter] << " for pi0 was not successful after maxiter=" 
                            << maxiter << " times! exiting..." << endl;}
                        exit(1);
                    }
...
float extractAsymmetries(
    ...
    map<string, TH1F*> diagnosticHists={
    ...
    TCanvas* c1 = new TCanvas("","",1440,600);
    TBox* box = new TBox();
...
//        string dataFileName=folder+"degALL_data_"+dataSetTag[iData]+"_BA_treeFlat_DSelector.root";
        string dataFileName=folder+"degALL_data_"+dataSetTag[iData]+"_looseDREventSelectSyst_treeFlat_DSelector.root";
        ...
        for(auto variable: variables){
            ...
            array_variable_map[dataSetTag[iData]+"_"+variable].reserve(nentries);
        ...
        for (Long64_t ientry=0; ientry<nentries; ++ientry){
            ...
            if ( (ientry%50000)==0 ){
                ...
                cout << ientry << "/" << nentries << "  elapsed time: " << elapsed.count() << endl;
            }
            ...
            selected=1;
            for (auto eventSel: defSels.selections){
                selectionVariable=*variable_map[eventSel.variable]; 
...
//                cout << " " << selected << " " << dataSetTag[iData]+"_"+eventSel.variable << " " << selectionVariable << endl;
            ...
            //    exit(1);
            //}
            nentries_preDefaultSelects+=AccWeight*weightBS;
            ...
            if(selectPi0*selectEta){ // if(insideEllipse)
                for(string variable: variables){
                    if ((variable==args.weightVar)*(args.weightVar!="AccWeight")) // overwrite the weights variable with the calculated
                        array_variable_map[dataSetTag[iData]+"_"+variable].push_back(AccWeight*weightBS); 
                    else
                ...
                dHist_Mpi0->Fill(*variable_map["Mpi0"],AccWeight);
                dHist_Meta->Fill(*variable_map["Meta"],AccWeight);
                // Fill the diagnostic histograms with accidental weights
                for (auto const& ele: diagnosticHists)
                    ele.second->Fill(*variable_map[ele.first],AccWeight);
                signalIntegral+=AccWeight*weightBS;
                nentries_postDefaultSelects+=AccWeight*weightBS;
            }
        }
    ...
    c1->SaveAs(("fitAsymmetryPlots_results/mass_plots"+evtTag+sbTag+".pdf").c_str());
    ...
    for (auto const& ele: diagnosticHists){
        ...
        c1->SaveAs(("fitAsymmetryPlots_results/diagnostics/"+ele.first+evtTag+sbTag+".png").c_str());
    }

    ofstream saveCsv;
    saveCsv.open(("fitAsymmetryPlots_results/results"+evtTag+sbTag+".csv").c_str());
...
//    //   BINNED IN U3
//    ////////////////////////////////////////////////////////////
//    string fitFolder="fitAsymmetryPlots_results/binned_u3"+evtTag+sbTag+"/";
...
//    //   BINNED IN PI0P
//    ////////////////////////////////////////////////////////////
//    fitFolder="fitAsymmetryPlots_results/binned_spi0p"+evtTag+sbTag+"/";
//    gSystem->Exec(("mkdir -p "+fitFolder).c_str());
...
//    //   BINNED IN METAP
//    ////////////////////////////////////////////////////////////
//    fitFolder="fitAsymmetryPlots_results/binned_setap"+evtTag+sbTag+"/";
//    gSystem->Exec(("mkdir -p "+fitFolder).c_str());
...
//    //   BINNED IN MPI0ETA
//    ////////////////////////////////////////////////////////////
//    fitFolder="fitAsymmetryPlots_results/binned_s12"+evtTag+sbTag+"/";
//    gSystem->Exec(("mkdir -p "+fitFolder).c_str());
